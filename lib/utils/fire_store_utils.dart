...
class FireStoreUtils {
  ...
  Stream<List<OrderModel>> getOrders(DriverUserModel driverUserModel, double? latitude, double? longLatitude) async* {
    ...
    // Special handling for test service IDs
    if (driverUserModel.serviceId == "test_service_id") {
      // For test service ID, get all rides without zone filter
      query = fireStore
          .collection(CollectionName.orders)
          .where('status', isEqualTo: Constant.ridePlaced);
    } else {
      // Normal query with service and zone filters
      query = fireStore
          .collection(CollectionName.orders)
    ...
    stream.listen((List<DocumentSnapshot> documentList) {
      ...
      for (var document in documentList) {
        ...
        // For test orders, always include them regardless of driver ID
        if (orderModel.serviceId == "test_service_id") {
          ordersList.add(orderModel);
        } 
        // For normal orders, follow standard logic
        else if (orderModel.acceptedDriverId != null && orderModel.acceptedDriverId!.isNotEmpty) {
